...
export default class MetamaskController extends EventEmitter {
  ...
  async requestAddNetworkApproval(customRpc, origin) {
    try {
      ...
      if (
        !(
          [EVENT.SOURCE.NETWORK.POPULAR_NETWORK_LIST, ORIGIN_METAMASK].includes(
            origin,
  ...
  /**
   * @typedef {import('./controllers/network/network-controller').NetworkConfiguration} NetworkConfiguration
   *
   ...
   * Then if setActive option is true sets the network as active and configures the provider accordingly.
   * @param {NetworkConfiguration} NetworkConfiguration - The network configuration to add.
   * @param {object} options - object with options such as "source" for event metrics and "setActive"
  ...
  async upsertNetworkConfiguration(
    { chainId, chainName, rpcUrl, ticker, rpcPrefs },
